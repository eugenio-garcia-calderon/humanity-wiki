...
const BLOQUES: { titulo: string; nota?: string; tareas: { texto: string; estado: Estado }[] }[] = [
  ...
  },
  {
    ...
    tareas: [
      ...
      { texto: 'Reparto mensual: construido y ejecutable; cada ejecución queda en el libro con el mes como entidad y no puede repetirse (índice único)', estado: 'hecha' },
      { texto: 'Reparto AUTOMÁTICO y modelo vigente (23-08-2026): el servidor paga el mes cerrado el día 1 sin que nadie pulse nada; 1.000 puntos fijos a cada persona verificada y activa (≥ 3 días de uso en el mes) + bote variable de 1.000 por reputación social del mes; regalo de bienvenida de 5.000 puntos; enviar puntos exige cuenta verificada', estado: 'en_revision' },
      { texto: 'Caducidad (10 años) e inactividad (24 meses): construidas. Barrido cada 6 h detrás de PUNTOS_CADUCIDAD (off = solo calcula; avisar = avisos por la campana a 30/7 días y a 90 días; on = avisa y ejecuta con apuntes contrarios). La fecha de conservación se ve bajo el saldo', estado: 'en_revision' },
      { texto: 'OJO (23-08-2026): con la bienvenida de 5.000 y las transferencias encendidas, el 403 «para enviar puntos hace falta una cuenta verificada» es hoy la ÚNICA barrera entre «cuenta nueva» y «5.000 puntos que se pueden mover». Probado en local por HTTP; en vivo lo prueba Eugenio con una cuenta sin verificar', estado: 'en_revision' },
      { texto: 'Unidad de referencia de precios (23-08-2026): 1 punto = una tarea simple con el modelo de IA más barato; el resto de la cesta son múltiplos publicados en /api/tokenomics/precios', estado: 'hecha' },
      { texto: 'Avisos y barrido de caducidad (10 años) e inactividad (24 meses) — los términos ya lo recogen; falta construirlo', estado: 'pendiente' },
      { texto: 'Puntos como descuento en el carrito, hasta el 100 %: construido detrás del interruptor PUNTOS_DESCUENTO (apagado). El vendedor cobra en puntos la parte pagada con puntos y opta producto a producto — ese es el abanico limitado del piloto', estado: 'en_revision' },
