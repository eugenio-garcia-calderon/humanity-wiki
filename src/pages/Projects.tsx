...
export default function ProjectsPage() {
  ...
  const filteredObjectives = objectives.filter(obj => {
    const objProjects = projects.filter(p => p.objective_ids && p.objective_ids.includes(obj.id));
    return objProjects.some(p => p.name.toLowerCase().includes(searchTerm.toLowerCase()));
  });
    ...
    <div key={updateCounter} className="space-y-8 animate-in fade-in duration-500 pb-12">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm text-slate-500 max-w-2xl">
          Explora los diferentes proyectos agrupados por objectives principales.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-12">
        {(searchTerm ? filteredObjectives : objectives).map(obj => {
          let objprojects = projects.filter(c => c.objective_ids && c.objective_ids.includes(obj.id) && c.name.toLowerCase().includes(searchTerm.toLowerCase()));
          
          if (selectedTerritoryId) {
            objprojects = objprojects.filter(c => c.territory_id === (selectedTerritoryId));
          }
                ...
                {objprojects.map(project => (
                  ...
                  >
                    <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity admin-menu-trigger z-10">
                      <AdminMenu onEdit={() => {
                        openEdit('Proyecto', { image: '', solution_ids: [], objective_ids: [], territory_id: '', ...project }, (data) => {
                          Object.assign(project, data);
                          triggerUpdate();
                        }, (data) => {
                          const idx = projects.findIndex(c => c.id === data.id);
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">PROYECTO</span>
                    </div>
                    {project.image && (
                    ...
                    {project.organization_ids && project.organization_ids.length > 0 && (
                      <div className="mt-4 pt-3 border-t border-slate-100 flex flex-col gap-1.5 text-[10px]">
                        <span className="font-bold text-slate-400 uppercase tracking-widest">Organización</span>
                        <div className="flex flex-wrap gap-1">
                          {project.organization_ids.map(orgId => {
                            const org = organizations.find(o => o.id === orgId);
                            return org ? (
                              <Link 
                                key={orgId} 
                                to={`/organizaciones/${slugify(org.name)}`}
                                className="text-emerald-600 bg-emerald-50 hover:bg-emerald-100 px-2 py-1 rounded transition-colors"
                    ...
                    {project.territory_id && (
                      <div className="mt-4 pt-3 border-t border-slate-100 flex items-center gap-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                        ...
                        {territories.find(t => t.id === project.territory_id)?.name || 'Territorio'}
                      </div>
                    )}
                  </div>
                ))}
        ...
        {/* Unassigned projects */}
        {(() => {
          let unassigned = projects.filter(c => (!c.objective_ids || c.objective_ids.length === 0) && c.name.toLowerCase().includes(searchTerm.toLowerCase()));
          if (selectedTerritoryId) {
                  ...
                  >
                    ...
                    {project.image && (
                      <div className="w-full h-32 mb-3 rounded-lg overflow-hidden bg-slate-200">
                        <img src={project.image} alt={project.name} className="w-full h-full object-cover opacity-75 group-hover:opacity-100 transition-opacity" />
                      </div>
                    )}
                    ...
                    {project.organization_ids && project.organization_ids.length > 0 && (
                      <div className="mt-4 pt-3 border-t border-slate-200 flex flex-col gap-1.5 text-[10px]">
                        <span className="font-bold text-slate-400 uppercase tracking-widest">Organización</span>
                        <div className="flex flex-wrap gap-1">
                          {project.organization_ids.map(orgId => {
                            const org = organizations.find(o => o.id === orgId);
                            return org ? (
                              <Link 
                                key={orgId} 
                                to={`/organizaciones/${slugify(org.name)}`}
                                className="text-slate-500 bg-slate-200 hover:bg-slate-300 px-2 py-1 rounded transition-colors"
                                onClick={(e) => e.stopPropagation()}
                              >
                                {org.name}
                              </Link>
