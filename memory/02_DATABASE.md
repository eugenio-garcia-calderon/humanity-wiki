...
> **Trampa conocida:** la columna `centroid` **existe en el esquema pero está vacía (`NULL`) para todas las filas** (comprobado el 2026-08-03, y confirmado de nuevo al añadir los 179 municipios de Madrid ese mismo día — tampoco se les rellenó `geometry`/`centroid` en la tabla). El centro real de cada territorio que usa el resto de la app viene de `seedTerritories` en `src/data/seed.ts` (campo `coordinates: [lng, lat]`), no de esta columna, y los polígonos reales de renderizado (país/región/municipio) se sirven desde archivos GeoJSON estáticos en `public/geo/` (ver `01_ARCHITECTURE.md`/changelog), no desde `territories.geometry`. Cualquier función nueva que necesite el centro de un territorio (distancias, "alrededores", etc.) debe leer `seedTerritories`, no `territories.centroid` — ver decisión del 2026-08-03 en `03_DECISIONS.md`.

> **179 municipios de la Comunidad de Madrid** (añadidos 2026-08-03, `type: 'municipality'`, `parent_id: 'T004'`): sus polígonos viven en `public/geo/madrid_municipios.json` (no en la tabla), sus IDs son `TMAD001`..`TMAD177` salvo los 2 que ya existían y se reutilizaron (`T014` Talamanca de Jarama, `T005` Montejo de la Sierra). Sus 14 puntuaciones de objetivo son datos simulados (Excel aportado por el usuario) — 13 de los 14 objetivos se resuelven vía `indicator_observations` reales (ver más abajo), y Salud (que no tiene ningún indicador en la BD) vía el diccionario mock `progress_by_territory` en `src/data/seed.ts`. Ver el changelog de esa fecha para la procedencia completa de los datos y `03_DECISIONS.md` para el razonamiento de emparejamiento.
