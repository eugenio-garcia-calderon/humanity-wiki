...
### 2026-08-24 — Los dos raíles, siempre puestos, y el submenú con flechita (Programador 2)
- Eugenio: «permite que ambos menús, el de la derecha y la izquierda, tengan el botón de volverse a plegar cuando se despliegan, y que el submenú no se despliegue con HOVER, sino que haya que hacer click en una flechita lateral… Y haz que además los menús laterales estén siempre visibles enseñando solo los iconos, y cuando se haga hover ahí, se despliegue».
- **Siempre puestos**: los dos raíles ya no aparecen sólo al pulsar su círculo; están en los dos bordes en modo iconos (56 px) y se despliegan al pasar el ratón. Antes, para saber qué hay en la aplicación había que probar un botón.
- **Dos gestos con dos destinos**: el NOMBRE lleva a la herramienta o al tema; la FLECHITA de al lado abre su submenú. Antes el submenú salía al pasar por encima, y bajar una lista de catorce era abrir trece paneles por el camino, cada uno pidiendo sus datos. Retirada la espera de 140 ms que tapaba eso: quitado el gesto, sobra la tirita.
- **Botón de plegar** en los dos, distinto de la chincheta: la chincheta dice «quédate abierto», plegar dice «ciérrate ahora». Hace falta precisamente porque existe la otra: un menú fijado ya no se cierra solo.
- **El raíl deja de flotar cuando hay un submenú abierto** y pasa a ocupar su sitio. Sin esto volvía el «se superpone el menú al submenú» que Eugenio ya señaló en su día: se había arreglado para el camino de pulsar el círculo y reaparecía por el camino nuevo.
- **Retirados los dos detectores de borde** de ayer. No porque la idea fuera mala: desde que los raíles están siempre puestos, **el menú ES el borde**, y tener dos cosas escuchando el mismo gesto para hacer cosas parecidas pero no iguales es peor que tener una.
- **`onElegir` ya no lo adivina el raíl**: llevaba `h.ruta.startsWith('/') ? navigate(h.ruta) : onElegir(h)`, y con eso los catorce temas se iban a `/objetivos/<id>` en vez de a `/explorar?objetivo=<id>`, que es la rejilla que pidió Eugenio. Adivinaba mal en 14 de 24 casos. Ahora decide quien lo pinta.
- **Dos fallos encontrados probando, no leyendo**:
  - El botón de plegar del raíl derecho **salía fuera de la pantalla** (`x = 1299` en una ventana de 1280). La cabecera no bajaba de lo que mide «Red de Conocimiento» (~190 px) y en 224 px no caben dos botones detrás; el raíl recorta lo que se sale, así que el botón existía, respondía al teclado y no se veía. `min-w-0` + `truncate`.
  - **Pulsar un círculo cerraba lo que su propio hover acababa de abrir**: para pulsarlo hay que pasar por encima, y pasar por encima ya lo abre, así que el interruptor lo leía como «segundo clic» y lo cerraba. Pulsar el botón no hacía nada visible, nunca. Ahora un clic sobre algo abierto por roce lo **confirma** (deja de cerrarse solo); cerrar es el segundo clic o el botón de plegar.
- Comprobado en el navegador a 1280 y a 375: iconos en los dos bordes, hover despliega, flecha abre el submenú al lado sin taparlo, plegar cierra, el círculo fija, y en móvil el cajón lleva las mismas flechas (sin ellas, los paneles se habrían quedado sin puerta en el teléfono).


### 2026-08-24 — Contrato de servicio de cobro y aceptación registrada (Programador 7)
- Eugenio (24-08): «crea tú esos contratos como si fueses mi asesor y avanza lo que puedas con la programación y la parte legal». Es lo que hace posible el pago único de un carrito con varias tiendas: sin acuerdo escrito con cada tienda, cobrar por ella no se puede. (Comprobado con fuentes: la exención de «agente comercial» de PSD2 no alcanza a quien retiene fondos y descuenta comisión antes de liquidar; la salida es apoyarse en un proveedor de pago autorizado — Stripe —, cuyo modelo «separate charges and transfers» está documentado literalmente para «un solo carrito con productos de varias empresas».)
- **Contrato v1.0** en `/avisos-legales?vista=cobro`: servicio de cobro **y no venta** (la venta sigue siendo de la tienda y ella factura); comisión 5 % / 2,5 % **con factura de la plataforma a la tienda** por el servicio; liquidación **a 14 días desde la entrega** (30 si nunca consta entregada); devoluciones y contracargos adelantados por la plataforma y compensados de las liquidaciones; retención cautelar motivada; 15 días para terminar; ley española. Aviso arriba del todo: lo redactó el equipo, no un abogado.
- Las dos decisiones que Eugenio no había tomado se resuelven del lado prudente y se dicen en voz alta en el propio contrato: contracargos a cargo final de la tienda, y liquidación diferida en vez de instantánea.
...
- **`src/server/ajustes.ts`**: una sola fuente con precedencia **base de datos → variable de entorno → valor por defecto del código** (el valor por defecto se queda en el código para que una base vacía arranque con cifras sensatas). Caché de 30 s que se vacía al guardar, así que un cambio se nota al instante. **17 cifras** en 6 grupos: comisiones, puntos que se emiten (bienvenida, fijo mensual, bote variable, días para ser activo, puntos por euro), pesos del reparto, topes, caducidad e inactividad, y comercio (plazo de devolución y días de liquidación).
- Todas las lecturas de `process.env` económicas de `puntos.ts`, `publicar.ts` y `stripe.ts` pasan por ahí: **cambiar una cifra en el panel cambia el cobro de verdad**, sin desplegar. `PLATFORM_FEE_BPS` deja de ser constante de arranque.
